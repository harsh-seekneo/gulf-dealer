...
export default function ListingsPage() {
  ...
  const loadVehicles = async () => {
    ...
    try {
      ...
      setTotalCount(data.totalCount ?? data.vehicles?.length ?? 0);
    } catch (err) {
    ...
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <a href="/vehicles" className="text-sm text-blue-600">View all Listings</a>
        </div>
      ...
      <ListingsTabs activeTab={activeTab} onChange={setActiveTab} totalCount={totalCount} />
